...
def Lista(root,lst):
    ...
    while i <= int(vezes) :
        ...
        e[i-1]["text"]= str(count) + "  " + lst[(i*2) - 2] + " " + " R$ " + lst[(i*2) - 1]
        largura += 275
        if i == new:
            altura += 35
            largura = 420
...
def CriaLista(janela,e2):
    ...
    adiciona = Button(janela, text="Adicionar", font="12",command=lambda: [Adiciona(), Criaarquivo(lista)], background="#fff")
    adiciona.place(x=10, y=270, width=200, height=30)
    finalizar = Button(janela, text="Finalizar", font="12",command=lambda: [Finalizar(adiciona,produto,preco,finalizar)], background="#fff")
...
def MudaLista(janela,e2):
    def Mudar():
        ...
        if produto1 == "":
            ...
            a = e2[int(mudar) - 1]
            a["text"] = mudar + "  " + lista[(int(mudar)*2) - 2] + " " + " R$ " + lista[(int(mudar)*2) - 1]
        if preco1 == "":
            None
        else:
    ...
    mudar = Button(janela, text="Adicionar", font="12", command= lambda: [Mudar(),Criaarquivo(lista)], background="#fff")
    mudar.place(x=10, y = 610, width=200, height= 30)
    finalizar = Button(janela, text="Finalizar", font= "12", command = lambda: [Finalizar(posicao,mudar,produto,preco,finalizar)], background="#fff")
...
def Deleta(janela,e2):
    ...
    def Teste(posicao,i,j):
        ...
        delete = posicao.get()
        deletando.append(lista[(2* int(delete)) - 2])
        deletando.append(lista[(2* int(delete)) - 1])

        while i < len(deletando):
            while j < len(lista):
                if deletando[i] == lista[j]:
                    lista.pop(j)
    ...
    deletar = Button(janela, text="Deletar", font="12", command= lambda: [Teste(posicao, i, j)], background="#fff")
    deletar.place(x=10, y = 740, width=200, height= 30)
    finalizar = Button(janela, text="Finalizar", font= "12", command = lambda: [Finalizar(posicao,deletar,finalizar)], background="#fff")
...
def Comprar(janela):
    ...
    Soma= 0
    while i <= vezes:
        Soma = Soma + float(lista[(int(ctxt[(i*2) - 2]) * 2) - 1]) * int(ctxt[(i*2) - 1])
        i += 1
...
Cria = Button(janela, text="Criar Lista", font="12",command=lambda: [CriaLista(janela, e2)], background="#fff")
Cria.place(x=10, y=150, width=300, height=30)

Aumenta = Button(janela,text = "Adicionar mais produtos a lista", font="12", command = lambda:[AumentaLista(janela, e2)],background="#fff")
Aumenta.place(x=10, y=320, width=300, height=30)

Muda = Button(janela,text = "Mudar os produtos da lista", font="12", command = lambda:[MudaLista(janela, e2)],background="#fff")
Muda.place(x=10, y=490, width=300, height=30)

Delete = Button(janela, text = "Deletar produto da lista", font ="12" , command = lambda:[Deleta(janela, e2)], background="#fff" )
...
Compra = Button(janela,text = "Adicionar ao Carrinho", font="12", command = lambda:[Carrinho(item,Quantidade)],background="#fff")
Compra.place(x=500, y=590, width=200, height=30)

Soma = Button(janela, text = "Finalizar Compra", font ="12" , command = lambda:[Comprar(janela)], background="#fff" )
